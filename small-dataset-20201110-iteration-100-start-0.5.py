...
fw = open("./results/small-dataset/small-dataset-20201112-in-out-account-trust-reliable-%d-%d-iteration-100-random.csv" % (beta1, beta2),"w")
fw.write("incount,outcount,trust,reliable\n")

for node in nodes:
    fw.write("%s,%s,%s,%s\n" %(str(incount_for_node[node]),str(outcount_for_node[node]),str(trust[node]), str(reliable[node])))
fw.close()

fw_confidence = open("./results/small-dataset/small-dataset-20201112-transasction-confidence-score-%d-%d-iteration-100-random.csv" % (beta1, beta2),"w")
